...
import { spyOnConsole } from "./spyOnConsole.js";
...
describe("Cache", () => {
  function itWithInitialData(
    ...
    callback: (...caches: InMemoryCacheRs[]) => any
  ) {
    const cachesList: InMemoryCacheRs[][] = [
      initialDataForCaches.map((data) =>
        new InMemoryCacheRs().restore(cloneDeep(data))
      ),
      initialDataForCaches.map((data) =>
        new InMemoryCacheRs({
          resultCaching: false,
  ...
  function itWithCacheConfig(
    ...
    callback: (cache: InMemoryCacheRs) => any
  ) {
    const caches = [
      new InMemoryCacheRs({
        ...config,
  ...
  describe("writeQuery", () => {
    itWithInitialData("will write some data to the store", [{}], (proxy) => {
      ...
      expect((proxy as InMemoryCacheRs).extract()).toEqual({
        ROOT_QUERY: {
          __typename: "Query",
          a: 1,
        },
    ...
    it("will write some deeply nested data to the store", () => {
      ...
      expect((cache as InMemoryCacheRs).extract()).toEqual({
        ROOT_QUERY: {
          __typename: "Query",
          a: 1,
          d: {
  ...
  describe("writeFragment", () => {
    ...
    itWithCacheConfig(
      ...
      (proxy) => {
        ...
        expect((proxy as InMemoryCacheRs).extract()).toMatchSnapshot();
        proxy.writeFragment({
  ...
  describe("cache.batch", () => {
    ...
    function watch(cache: InMemoryCacheRs, query: DocumentNode) {
...
describe("resultCacheMaxSize", () => {
  it("uses default max size on caches", () => {
    ...
    expect(cache["storeReader"]["executeSelectionSet"].options.max).toBe(
      DEFAULT_CACHE_SIZES["inMemoryCache.executeSelectionSet"]
    );
    expect(cache["getFragmentDoc"].options.max).toBe(
      DEFAULT_CACHE_SIZES["cache.fragmentQueryDocuments"]
    );
  });
});

describe("InMemoryCacheRs#broadcastWatches", function () {
...
describe("InMemoryCacheRs#modify", () => {
...
describe("ReactiveVar and makeVar", () => {
  function makeCacheAndVar(resultCaching: boolean) {
    ...
    const cache: InMemoryCacheRs = new InMemoryCacheRs({
      resultCaching,
      typePolicies: {
        Person: {
          fields: {
