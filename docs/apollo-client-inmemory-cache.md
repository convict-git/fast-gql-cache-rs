...
- [Part 0 — Orientation](#part-0--orientation)
  - [0.1 The one-paragraph mental model](#01-the-one-paragraph-mental-model)
  - [0.2 The blog's example, as the cache actually stores it](#02-the-blogs-example-as-the-cache-actually-stores-it)
  - [0.3 File map](#03-file-map)
  - [0.4 Vocabulary](#04-vocabulary)
  - [0.5 The whole machine in one diagram](#05-the-whole-machine-in-one-diagram)
- [Part 1 — Foundations](#part-1--foundations)
  - [1.1 `optimism` — memoization with automatic dependency tracking](#11-optimism--memoization-with-automatic-dependency-tracking)
    - [`wrap(fn, options)` → memoized function](#wrapfn-options--memoized-function)
    - [`Entry` — the dependency graph](#entry--the-dependency-graph)
    - [`dep()` — dependency leaves](#dep--dependency-leaves)
    - [`Slot` — dynamic scoping](#slot--dynamic-scoping)
  - [1.2 `@wry/trie` — tuples as stable object identities](#12-wrytrie--tuples-as-stable-object-identities)
  - [1.3 `@wry/caches` — the LRU behind every memo](#13-wrycaches--the-lru-behind-every-memo)
  - [1.4 `@wry/equality` — the change detector](#14-wryequality--the-change-detector)
  - [1.5 `DeepMerger` — merging with maximal structure sharing](#15-deepmerger--merging-with-maximal-structure-sharing)
  - [1.6 `canonicalStringify` — deterministic keys](#16-canonicalstringify--deterministic-keys)
  - [1.7 `maybeDeepFreeze` — the immutability contract](#17-maybedeepfreeze--the-immutability-contract)
- [Part 2 — The normalized store](#part-2--the-normalized-store)
  - [2.1 The layer chain](#21-the-layer-chain)
  - [2.2 Reading a field through the chain](#22-reading-a-field-through-the-chain)
  - [2.3 `NormalizedCacheObject` and `__META`](#23-normalizedcacheobject-and-__meta)
  - [2.4 `CacheGroup` — the dependency graph](#24-cachegroup--the-dependency-graph)
  - [2.5 State transitions of a single `(dataId, storeFieldName)`](#25-state-transitions-of-a-single-dataid-storefieldname)
  - [2.6 Writes: `merge` and `storeObjectReconciler`](#26-writes-merge-and-storeobjectreconciler)
  - [2.7 `modify` — user-controlled field surgery](#27-modify--user-controlled-field-surgery)
  - [2.8 `evict` — deletion across the layer chain](#28-evict--deletion-across-the-layer-chain)
  - [2.9 Garbage collection](#29-garbage-collection)
  - [2.10 Layer removal and replay](#210-layer-removal-and-replay)
- [Part 3 — `Policies`](#part-3--policies)
  - [3.1 Lazy materialisation and supertype inheritance](#31-lazy-materialisation-and-supertype-inheritance)
  - [3.2 Entity identity: `Policies.identify`](#32-entity-identity-policiesidentify)
    - [`defaultDataIdFromObject`](#defaultdataidfromobject)
    - [`keyFields` specifiers](#keyfields-specifiers)
  - [3.3 Field identity: `getStoreFieldName`](#33-field-identity-getstorefieldname)
    - [`keyArgs` specifiers](#keyargs-specifiers)
    - [Where `storeFieldName` is decided, end to end](#where-storefieldname-is-decided-end-to-end)
  - [3.4 `readField` — the field read entry point](#34-readfield--the-field-read-entry-point)
  - [3.5 Merge functions](#35-merge-functions)
    - [`mergeObjects`](#mergeobjects)
  - [3.6 `fragmentMatches` — type-condition resolution](#36-fragmentmatches--type-condition-resolution)
- [Part 4 — `StoreWriter`](#part-4--storewriter)
  - [4.1 `writeToStore` — the driver](#41-writetostore--the-driver)
  - [4.2 `processSelectionSet` — the recursive core](#42-processselectionset--the-recursive-core)
  - [4.3 `flattenFields` — field collection with directive tracking](#43-flattenfields--field-collection-with-directive-tracking)
  - [4.4 `processFieldValue` — scalars, arrays, recursion](#44-processfieldvalue--scalars-arrays-recursion)
  - [4.5 Identification and the `keyObject` back-channel](#45-identification-and-the-keyobject-back-channel)
  - [4.6 `MergeTree` and `applyMerges`](#46-mergetree-and-applymerges)
  - [4.7 The cycle breaker and the `isFresh` short-circuit](#47-the-cycle-breaker-and-the-isfresh-short-circuit)
  - [4.8 `warnAboutDataLoss`](#48-warnaboutdataloss)
  - [4.9 The full write, end to end](#49-the-full-write-end-to-end)
  - [4.10 Write-path state transitions](#410-write-path-state-transitions)
- [Part 5 — `StoreReader`](#part-5--storereader)
  - [5.1 The two memoized functions](#51-the-two-memoized-functions)
  - [5.2 `diffQueryAgainstStore`](#52-diffqueryagainststore)
  - [5.3 `execSelectionSetImpl`](#53-execselectionsetimpl)
  - [5.4 `execSubSelectedArrayImpl`](#54-execsubselectedarrayimpl)
  - [5.5 The missing tree](#55-the-missing-tree)
  - [5.6 Immutability and `knownResults`](#56-immutability-and-knownresults)
  - [5.7 What a read leaves behind](#57-what-a-read-leaves-behind)
  - [5.8 Reading with `optimistic: true`](#58-reading-with-optimistic-true)
- [Part 6 — Reactivity](#part-6--reactivity)
  - [6.1 `watch`](#61-watch)
  - [6.2 `broadcastWatch` and the equality gate](#62-broadcastwatch-and-the-equality-gate)
  - [6.3 `txCount` — broadcast batching](#63-txcount--broadcast-batching)
  - [6.4 `batch` — the transactional API](#64-batch--the-transactional-api)
    - [The three `optimistic` modes](#the-three-optimistic-modes)
    - [The `alreadyDirty` dance](#the-alreadydirty-dance)
  - [6.5 Optimistic lifecycle, end to end](#65-optimistic-lifecycle-end-to-end)
  - [6.6 Reactive variables](#66-reactive-variables)
  - [6.7 `watchFragment` — the observable layer on top of `watch`](#67-watchfragment--the-observable-layer-on-top-of-watch)
- [Part 7 — Method-by-method reference](#part-7--method-by-method-reference)
  - [7.1 `read`](#71-read)
  - [7.2 `diff`](#72-diff)
  - [7.3 `write`](#73-write)
  - [7.4 `modify`](#74-modify)
  - [7.5 `evict`](#75-evict)
  - [7.6 `watch`](#76-watch)
  - [7.7 `batch` / `performTransaction`](#77-batch--performtransaction)
  - [7.8 `removeOptimistic`](#78-removeoptimistic)
  - [7.9 `gc`](#79-gc)
  - [7.10 `retain` / `release`](#710-retain--release)
  - [7.11 `extract`](#711-extract)
  - [7.12 `restore`](#712-restore)
  - [7.13 `reset`](#713-reset)
  - [7.14 `identify`](#714-identify)
  - [7.15 `transformDocument` / `transformForLink`](#715-transformdocument--transformforlink)
  - [7.16 `fragmentMatches` / `lookupFragment` / `resolvesClientField`](#716-fragmentmatches--lookupfragment--resolvesclientfield)
  - [7.17 The inherited convenience layer](#717-the-inherited-convenience-layer)
  - [7.18 What `InMemoryCache` deliberately does *not* implement](#718-what-inmemorycache-deliberately-does-not-implement)
- [Part 8 — The cache in the Apollo Client pipeline](#part-8--the-cache-in-the-apollo-client-pipeline)
  - [8.0 The call map](#80-the-call-map)
  - [8.1 Document transforms — what the cache sees is not what you wrote](#81-document-transforms--what-the-cache-sees-is-not-what-you-wrote)
  - [8.2 `ObservableQuery` — the cache's principal client](#82-observablequery--the-caches-principal-client)
    - [The watch callback's four gates](#the-watch-callbacks-four-gates)
  - [8.3 Fetch policies as a cache-interaction table](#83-fetch-policies-as-a-cache-interaction-table)
  - [8.4 `QueryInfo.markQueryResult` — the write path and the feud breaker](#84-queryinfomarkqueryresult--the-write-path-and-the-feud-breaker)
  - [8.5 Mutations — optimistic layer, final write, root-field scrub](#85-mutations--optimistic-layer-final-write-root-field-scrub)
  - [8.6 `refetchQueries` — the batch-and-collect protocol](#86-refetchqueries--the-batch-and-collect-protocol)
  - [8.7 Broadcast → notify → reobserve](#87-broadcast--notify--reobserve)
  - [8.8 Local state and `@client` fields](#88-local-state-and-client-fields)
  - [8.9 Data masking](#89-data-masking)
  - [8.10 `resetStore` and `clearStore`](#810-resetstore-and-clearstore)
  - [8.11 Memory internals — the cache's own telemetry](#811-memory-internals--the-caches-own-telemetry)
- [Part 9 — Invariants and a re-implementation checklist](#part-9--invariants-and-a-re-implementation-checklist)
  - [9.1 The invariants](#91-the-invariants)
    - [Storage](#storage)
    - [Layers](#layers)
    - [Dependency tracking and reactivity](#dependency-tracking-and-reactivity)
    - [Reads](#reads)
    - [Writes](#writes)
    - [Policies](#policies)
  - [9.2 Build order for a re-implementation](#92-build-order-for-a-re-implementation)
  - [9.3 Cross-boundary requirements](#93-cross-boundary-requirements)
  - [9.4 The minimum viable surface](#94-the-minimum-viable-surface)
  - [9.5 Where to look when something is wrong](#95-where-to-look-when-something-is-wrong)
